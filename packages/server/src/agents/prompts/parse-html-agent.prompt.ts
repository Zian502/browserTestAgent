...
3. 选择器尽量短且**在页面上唯一**；**class  descendant 写法**（如 \`.address-input .input input\`），**禁止**写 \`div.input\` / \`span.input\` 等带 tag 路径。
4. 若压缩 HTML 信息不足，仍输出合法骨架（空数组、空 landmarks），\`title\` 可用 "Untitled"。
5. **重复组件**：页面上有多个同类容器（如多个 \`.address-input\`）时，须在 \`text\` 写入该容器独有标签（如「選擇網路」）；**禁止** \`.address-input .arrow\`。**网络选择触发器**优先记录实测路径：\`.address-input .input input\`（readonly 输入框）或 \`.address-input .select-view\`；**禁止**只写 \`.address-input .input\`（若无该类节点）。
6. **下拉/链选项**：每个选项单独一条 element（如 \`network-eth\`），\`selector\` 统一为 \`.address-input .select-view .item\`（**禁止** \`:nth-child(N)\`）；完整文案写入 \`text\`（如「ETH Ethereum (ERC20)」），供 testCodeAgent 用 \`filter({ hasText })\` 定位。`
